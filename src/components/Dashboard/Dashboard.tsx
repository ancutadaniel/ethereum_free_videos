...
const Dashboard: FC<DashboardProps> = ({ selectedItem, className }) => {
  const { provider, contract, isInitialized } = useBlockchain();
    ...
    <div className={`flex flex-col bg-green-100 ${className}`}>
      ...
      <div className="flex flex-col md:flex-row flex-grow h-full mx-4 mb-4 space-y-4 md:space-y-0 md:space-x-4">
        ...
        <div className="md:w-7/12 w-full order-2 md:order-1 h-full">
          <div className="bg-white p-4 rounded shadow-md h-full flex">
            {loading ? (
              <p>Loading videos...</p>
            ) : error ? (
              ...
              selectedVideo && (
                <div className="mt-4 text-left w-full">
                  ...
                  <div className="relative w-full overflow-hidden rounded-md shadow-lg" style={{ paddingTop: '56.25%' }}>
                    <video controls className="absolute top-0 left-0 w-full h-full">
                      <source src={`${IPFS_BASE_URL}${selectedVideo.hash}`} type="video/mp4" />
